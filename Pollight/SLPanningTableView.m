...
static const NSInteger PAN_OPEN_X = -180;
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    TodaysPollSlidingAnswerTableCell *cell = (TodaysPollSlidingAnswerTableCell *)[panGestureRecognizer view];
...
- (void)handleSparrowPan:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    float compare;
    float finalX;
    NSIndexPath *indexPath = [self indexPathForCell:(TodaysPollSlidingAnswerTableCell *)[panGestureRecognizer view] ];
    UIView *view = ((TodaysPollSlidingAnswerTableCell *)panGestureRecognizer.view).panningView;
    
    switch ([panGestureRecognizer state]) {
        case UIGestureRecognizerStateBegan:
            if (self.openCellIndexPath.section != indexPath.section || self.openCellIndexPath.row != indexPath.row) {
                [self snapView:((TodaysPollSlidingAnswerTableCell *)[self cellForRowAtIndexPath:self.openCellIndexPath]).panningView toX:PAN_CLOSED_X animated:YES];
        ...
        case UIGestureRecognizerStateEnded:
            ...
            if (finalX == PAN_CLOSED_X) {
                ...
                [self setOpenCellIndexPath:[self indexPathForCell:(TodaysPollSlidingAnswerTableCell *)panGestureRecognizer.view]];
            }
            break;
...
-(void)closeSlidedCell
{    
    UIView * view = ((TodaysPollSlidingAnswerTableCell *)[self cellForRowAtIndexPath:self.openCellIndexPath]).panningView;
    
    view.center = CGPointMake(339, 56);
    
    [self setOpenCellIndexPath:nil];
    [self setOpenCellLastTX:0];
}

-(void)setSlidingDisabled:(BOOL)slidingDisabled
{
    _slidingDisabled = slidingDisabled;
    if (slidingDisabled) {
        [self closeSlidedCell];
    }
}
